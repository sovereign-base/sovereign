...
- [ ] **ENG-02**: An agent can read and update project state via `sovereign-tools state load|save`, with field-level patching (not whole-file rewrites).
- [ ] **ENG-03**: An agent can open and pass phase gates via `sovereign-tools gate open|pass <phase>` using append-only writes.
- [ ] **ENG-04**: An agent can commit state/planning artifacts via `sovereign-tools commit "<msg>" --files ...` that respects `commit_docs` config and gitignore status.
- [ ] **ENG-05**: An agent can resolve a subagent's model via `sovereign-tools model <agent>` according to the configured model profile.
...
- [ ] **ENG-07**: `sovereign-tools validate skills` lints SKILL.md frontmatter (name ≤64 chars, lowercase-hyphen, no "claude"/"anthropic"; description within length cap) and exits non-zero on violations.
...
- [ ] **STATE-02**: MANIFEST.md is regenerated by the engine on every `state save` and stays within a ~500-token budget (engine-derived, never hand-maintained).
- [ ] **STATE-03**: `.sovereign/` is committed to git by default; the engine auto-detects when it is gitignored and skips commits accordingly.
