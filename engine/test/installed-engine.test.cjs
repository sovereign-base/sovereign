 ...
 *   - Test C — a non-init command (`state load`) exits 0 from the installed
 *     location, proving require('./lib/...') resolves there.
...
// ─── Test C — a non-init command resolves require('./lib/...') from there ────

test('installed-engine Test C: state load via the installed path exits 0', () => {
  const dir = mkProject();
  try {
    runInstall({ cwd: dir, target: 'project', mode: 'full', packageRoot: ENGINE_ROOT });
    ...
    const res = spawnSync(
      process.execPath,
      ['.claude/sovereign-engine/sovereign-tools.cjs', 'state', 'load'],
      { cwd: dir, encoding: 'utf-8' }
    );
    assert.equal(res.status, 0, res.stderr);
  } finally {
